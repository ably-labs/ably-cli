...
static hidden = false
  ...
  async run(): Promise<void> {
    ...
    this.log()
    this.log('1. Your Ably Account ID')
    this.log('2. An Access Token')
    this.log()
    ...
    this.log()
    this.globalConfig.controlApiToken = await CliUx.ux.prompt('What is Control Access token?')

    this.log('And the instructions here to get your Account ID:')
    await CliUx.ux.url('https://ably.com/docs/control-api#ids', 'https://ably.com/docs/control-api#idsn')
    this.log()
    this.globalConfig.accountId = await CliUx.ux.prompt('What is Account ID?')
